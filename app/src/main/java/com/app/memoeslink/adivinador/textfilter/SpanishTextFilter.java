...
public class SpanishTextFilter implements TextFilter {
    ...
    public static final String SEXUAL_INTERCOURSE_PROFANITY_REGEX = "(" + "sexo|sexual(es)?|coitos?|(coital(es)?)|fornicador([ea]s)?|fornicación|fornicatori[oa]s?|orgasmos?|orgásmic[oa]s?|genital(es)?|copular|cópular|copulatori[oa]s?|follar|follatori[oa]s?|follador(es|as?)?|foll(ón|ones|onas?)|folla(culo|negr[oa]|perr[oa])s?|orgías?|mamar|mamad(a|ita|ota)s?|erecci(ón|ones)|semen|eyacular|eyaculaci(ón|ones)|eyaculatori[oa]s?|anilingus" + ")";
    public static final String BODY_PROFANITY_REGEX = "(" + "cul([o0]|ito|ote|azo)s?|cul(ón|ones|onas?)|nalg(a|uita|ota)s?|nalgonas?|pedorr(o|ito|ote|azo)s?|pedorr(ón|ones|onas?)|an(o|ito|ote)s?|anal(es)?|rectal(es)?|entrepiernas?|pubis|púbic[oa]s?" + ")";
    public static final String BREASTS_PROFANITY_REGEX = "(" + "t[e3]t([a4]|ita|ota|aza)s?|tet(ón|ones|onas?|ud[oa]s?)|tetamen|pechos?|senos?|pez(ón|ones)|tet(ita|ota)s?|pech(ito|ote)s?|chich(ita|ota)s?|chich(ón|ones|onas?)|mamas?|mamari[oa]s?" + ")";
    ...
    public static final String FECAL_MATTER_PROFANITY_REGEX = "(" + "mierd(a|ita|ota|aza)s?|mierd(ón|ones|onas?)|mierder[oa]s?|cagad([oa]|it[oa]|ot[ea])s?|cag(ón|ones|onas?)|ca(ca|quita|cota)s?|comemierdas?|he(z|ces)|moj(ón|ones|on(cito|zote)s?)|soretes?" + ")";
    public static final String MASTURBATION_PROFANITY_REGEX = "(" + "masturbaci(ón|ones)|masturbatori[oa]s?|masturbador(es|as)?|onanismos?|puñetas?|puñeter[oa]s?|chaqueter[oa]s?|pajer[oa]s?" + ")";
    public static final String HOMOSEXUALITY_PROFANITY_REGEX = "(" + "put([oa]|it[oa]|ot[ea]|az[oa])s?|put(ón|ones|onas?)|maric(as?|ón|onas?|ones|on(cito|zote)s?)|mariquitas?|trolazos?|jot(o|it[oa]|ot[ea])s?|jotol(ón|ones)|jotaz[oa]s?|jototes?|joterías?|amariconad[oa]s?|muerdealmohadas?|mariconaz[oa]s?|jotísim[oa]s?|sodomitas?|transexual(es)?" + ")";
    ...
    public static final String RACIST_OR_XENOPHOBIC_SLURS_REGEX = "(" + "sudacas?|prietos?" + ")";
    ...
    public static final String CENSOR_REGEX = "(?<start>^|\\s+|[^\\p{IsLatin}])(?<text>" +
            ...
            "|" + RACIST_OR_XENOPHOBIC_SLURS_REGEX +
